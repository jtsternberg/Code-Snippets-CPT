var snippetcpt_ace_viewer = ( function( window, document, $, undefined ){
	...
	var snippet_viewers = [];
	var ace_l10n = window.ace_editor_front_end_globals || {};
	...
	var new_viewer = function( ) {
		...
		snippet_viewer.init = function(){
			...
			if ( ace_l10n.theme ){
				editor.setTheme( ace_l10n.theme );
			} else if ( ace_l10n.default_theme ) {
				editor.setTheme( ace_l10n.default_theme );
			} else {
			...
			} else if( ace_l10n.default_lang ) {
				editor.getSession().setMode( ace_l10n.default_lang );
			} else {
