...
( function( window, document, $, cpt, undefined ) {
	...
	cpt.addIcons = function( icons ) {
		...
		$c.pretty.each( function() {
			...
			for ( var i = 0; i < icons.length; i++ ) {
				if ( 'fullscreen' === icons[i] && $this.parent( '.footer-prettyprint' ).length ) {
					icons[i] = 'close';
				}
	...
	cpt.fullScreenInit = function() {
		...
		$c.footer = $( '.footer-prettyprint' );
		if ( ! $c.footer.length ) {
			$c.footer = $( '<div class="footer-prettyprint snippet-hidden"></div>' ).appendTo( $c.body );
		}
	...
	cpt.openSnippet = function( evt ) {
		...
		var $snippet = $(this).parents( '.pretty-print-wrap' ).clone();
